...
export default function DepartmentChat() {
  ...
  const { messages, loadingHistory, isOnline, offlineQueue, sendMessage } = useDepartmentChat(activeDept);
  ...
  // Optimized auto-scroll - only smooth scroll for new messages, not bulk loads
  useEffect(() => {
    ...
    <div className="flex h-[calc(100vh-120px)] bg-slate-50 dark:bg-[#0B1120] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden animate-fade-in">
      ...
      <div className="w-64 sm:w-80 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111827] flex flex-col shrink-0">
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 shrink-0">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Select a department to chat</p>
        </div>
          ...
          <>
            ...
            <div className="h-16 px-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-[#111827] shrink-0">
              ...
              {isOnline && offlineQueue.length > 0 && (
                <span className="text-xs text-amber-500">Sending queued ({offlineQueue.length})...</span>
              )}
            </div>
